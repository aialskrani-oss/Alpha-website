import { Router } from "express";
  ...
  router.get("/", async (_req, res) => {
    try {
      const [sectionRows, platformRows, newsRows] = await Promise.all([
      ...
      res.json({ totalSections, totalPlatforms, totalNews, featuredPlatforms, activePlatforms, platformsByType, platformsBySection });
    } catch (err) {
